...
NAME
		...
		void Purple::reset(const Position& home) - resets all the variables back to the original values

SYNOPSIS

		Blue::reset(const Position& home)
		   home            --> a Position void Purple::reset(const Position& home) variable that stores the x and y cooridinates of the ghost home or the starting position
...
void Purple::reset(const Position& home)
{
...
NAME
		...
		void Purple::draw(sf::RenderWindow& window)
...
void Purple::draw(sf::RenderWindow& window)
{
	...
	sf::Texture texture;
	texture.loadFromFile("purple.png");
	sf::Texture scared;
...
NAME
		...
		void Purple::ghost_movement(std::array<std::array<Cell, g_MAP_HEIGHT>, g_MAP_WIDTH>& map, Pacman& pacman, int floor)
...
NAME
		...
		void Purple::new_target(int pacman_direction, const Position& pacman_position, int floor) 

SYNOPSIS

		void Purple::new_target(int i_pacman_direction, const Position& i_pacman_position, int floor) 
...
DESCRIPTION
		...
		This function changes the target tile of the purple slime depending on the movement mode and the floor or maze map
...
void Purple::new_target(int pacman_direction, const Position& pacman_position, int floor) {
	...
	{
		//the purple slime is the same as the pink ghost in that it targets 2 spaces ahead of pacman
		m_target = pacman_position;
		switch (pacman_direction) {
		case 0:
			m_target.x += g_CELL_SIZE * 2;
			break;
		case 1:
			m_target.y -= g_CELL_SIZE * 2;
			break;
		case 2:
			m_target.x -= g_CELL_SIZE * 2;
			break;
