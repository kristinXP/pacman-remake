...
void Red::reset(const Position& home)
{
...
*/
/**/
void Red::draw(sf::RenderWindow& window)
{
...
NAME
		...
		void Red::ghost_movement(std::array<std::array<Cell, g_MAP_HEIGHT>, g_MAP_WIDTH>& map, Pacman& pacman, int floor)
...
NAME
		...
		void Red::new_target(int pacman_direction, const Position& pacman_position, int floor)
...
void Red::new_target(int pacman_direction, const Position& pacman_position, int floor) {
	...
	else if ("scatter" == m_movement_mode) //The scatter mode
	{
		...
		case 0:
		{
			m_target = { g_CELL_SIZE * (g_MAP_WIDTH - 1), 0 };
			break;
		}
		case 1:
		{
		...
		case 3:
		{
			m_target = { g_CELL_SIZE * (g_MAP_WIDTH - 4), 0 };
			break;
		}
		case 4:
		{
	...
	else if ("chase" == m_movement_mode)//The chase mode
	{
		...
		//target is aways pacman's position
