//
// Class to provide and control the game over menu - intended selected options font is changed to red
//
...
public:
	...
	GameOverSelection(float width, float height) {
	...
	//changes the font color of the first menu option to red and sets the selected item to the top option
	void MoveUp() {

		//bound checking to make sure the selected elemnt does not exceed the number of selectable options
		if (m_selected - 1 >= 0)
		{
			//changes the color of the bottom option to white 
			menu[m_selected].setFillColor(sf::Color::White);

			//deincrements the selected count to change the selected menu option
			m_selected--;

			//changes the color of the top option text to red  
			menu[m_selected].setFillColor(sf::Color::Red);

		}
	};

	//changes the font color of the second menu option to red and sets the selected item to the bottom option
	void MoveDown() {

		//bound checking to make sure the selected elemnt does not exceed the number of selectable options
		if (m_selected + 1 <= 1)
		{
			//changes the color of the top option to white 
			menu[m_selected].setFillColor(sf::Color::White);

			//deincrements the selected count to change the selected menu option
			m_selected++;

			//changes the color of the bottom option text to red  
			menu[m_selected].setFillColor(sf::Color::Red);
